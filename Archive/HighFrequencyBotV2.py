...
class TradingBot:
    ...
    def update_params(self):
        ...
        self.std_coeff = self.params['std_coeff'] * abs(float(sd_adjustment))
    ...
    def get_trend_data(self, coin, fr_adjustment_factor):
        ...
        sell_bound = max((coin_data['buy_spread_ma_M'] + coin_data['sell_spread_ma_M'])/2 + coin_data['sell_spread_sd_L']*self.std_coeff, self.min_width/2 + (coin_data['buy_spread_ma_M'] + coin_data['sell_spread_ma_M'])/2) + max(fr_adjustment_factor, 0)
        buy_bound = min((coin_data['buy_spread_ma_M'] + coin_data['sell_spread_ma_M'])/2 - coin_data['buy_spread_sd_L']*self.std_coeff, (coin_data['buy_spread_ma_M'] + coin_data['sell_spread_ma_M'])/2 - self.min_width/2) + min(fr_adjustment_factor, 0)

        return sell_bound, buy_bound, coin_data, coin_data['buy_spread_ma_M'], coin_data['sell_spread_ma_M'], coin_data['buy_spread_sd_L'], coin_data['sell_spread_sd_L']
                ...
                try:
                    ...
                    if self.current_okx_order_id is None:
                        ...
                        # If the sides didn't change:
                            ...
                            if side == 'sell':
                                if ma_entry_spread > self.entry_bound:
                                    spread_difference = 0   # make sure modify is true
                                else:
                                    spread_difference = self.entry_bound - ma_entry_spread
                                modify = spread_difference <= 0.02
                            else:  # side == 'buy'
                                if ma_exit_spread < self.exit_bound:
                                    spread_difference = 0   # make sure modify is true
                                else:
                                    spread_difference = ma_exit_spread - self.exit_bound
                                modify = spread_difference <= 0.02  # -35%
                    ...
                    if 'Your order has already been filled or canceled' in str(e):
                        # TODO: How to differentiate between a filled order and a cancelled or missing one?
                        self.current_okx_order_id = None
                        ...
                        logger.info(
                            f'Order filled | {self.symbol} | {self.previous_side} | Notional: {self.notional_per_trade} | Position: {round(current_position, 2)} | Abs Position: {round(abs_current_position, 2)} | Max Notional: {self.max_notional} | Change Since Update: {round(self.position_change_since_update, 2)} | Time of Trade: {self.timeoftrade} | Current Time: {datetime.now()} | latency: {(datetime.now() - self.timeoftrade).total_seconds() * 1000}')
                        print(
