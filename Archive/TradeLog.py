...
def fetch_usdt_perp_trades(exchange, exchange_id, since):
    all_trades = []
    ...
    for symbol in usdt_perp_symbols:
        ...
        try:
            # use since to pull info before the date.
            # use until to pull information until the date.
...
def run_trading_bot():
    ...
    binance_trades = fetch_usdt_perp_trades(binance, 'binance', since_timestamp)
    print("Fetching OKX USDT perpetual futures trades...")
    okx_trades = fetch_usdt_perp_trades(okx, 'okx', since_timestamp)
