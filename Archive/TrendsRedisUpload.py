...
class TrendsRedisUpload:
    ...
    async def read_data_batch(self, batch_size=10000):
        query = 'SELECT * FROM "spreads" WHERE timestamp > "2024-09-10 00:00:00"'
    ...
    async def process_and_upload(self):
        ...
        if not all_data.empty:
            ...
            ma_range_df = await self.calculate_ma_range(all_data, window_m=500, window_l=35)
            print(ma_range_df)
...
def update_trends():
    try:
        database_path = r'C:\Users\Lewis Lu\PycharmProjects\Hyperliquid\CrossExchangeMarketMaking\LiveTestingV2\FundingRateArbitrage\arbitrage_data.db'
