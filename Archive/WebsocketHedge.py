...
async def stop_run_bot():
    # Send in a true to stop the bot from continuing to trade
...
class WebsocketHedge:
    def __init__(self, binance_client: AsyncClient, time_sync: TimeSync):
    ...
    async def hedge_on_binance(self, symbol, side, amount, cli_ord_id):
        ...
        # TODO: Any way to quickly get min notional? Currently only looking at min step sizes.
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                order = await self.binance_client.futures_create_order(
                    ...
                    timestamp=timestamp  # Add the adjusted timestamp
                )
                logger.info(f"Binance hedge order filled: {order}")

                # Update the unhedged amount after successful hedge
                ...
                await reset_error_count()
                return  # Exit the function after successful order
                ...
                if "Server is currently overloaded" in str(e):
                    ...
                    # For all other errors, log and return immediately
            ...
            await check_and_update_error_state(error_occurred=False)  # Reset count on overall success
