...
class TrendsRedisUpload:
    ...
    def calculate_spread(df):
        df['sell_spread'] = (df['okx_bid1'] - df['binance_ask1']) / df['binance_ask1'] * 100
        df['buy_spread'] = (df['okx_ask1'] - df['binance_bid1']) / df['binance_bid1'] * 100
        return df
...
def update_trends():
    try:
        connection_string = "Driver={ODBC Driver 18 for SQL Server};Server=tcp:theosql.database.windows.net,1433;Database=arbitrage_db_2024-03-22T23-30Z;Uid=THEOsql;Pwd=THEOBullRun2024!;Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30;"
