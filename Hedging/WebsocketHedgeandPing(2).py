...
logger = config.setup_logger('WebsocketHedge(2)')
...
async def perform_hedge_check():
    API_Hedge = APIHedge(config.HEDGE2)
...
class WebsocketHedge:
    ...
    async def process_order_update(self, order):
        ...
        if order.get('instId').replace('-USDT-SWAP', '') in config.HEDGE2:
            print(f"Order Update: Time lag - {time_lag} ms - {order}")
            if order.get("fillSz") != "0" and not order['clOrdId'].startswith("TTSpread"):
                if order.get('instId').replace('-USDT-SWAP', '') == 'RON':
                    symbol = 'RONIN-USDT-SWAP'
                    side = order["side"]
