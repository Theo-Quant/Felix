...
class TradingBot:
    ...
    def update_params(self):
        ...
        elif self.exchange_perp == 'hyperliquid':
            self.contract_size = 1
        ...
        if self.params['bybit_mark_price'] != 0:
            self.mark_price = self.params['bybit_mark_price']
        elif self.params['hyperliquid_mark_price'] != 0:
    ...
    async def place_limit_order(self, symbol, side, quantity, price):
        try:
            ...
            if self.exchange_perp == 'okx':
                ...
                order = await hyperliquid.create_order(symbol, "limit", side, quantity, price,
                                                   params={'newClientOrderId': client_order_id, 'postOnly': True, "limit": {"tif": "Alo"}})
                print('Hyperliquid Order', order)
                return order['id']
    ...
    async def edit_limit_order(self, order_id, symbol, side, quantity, price):
        try:
            if self.exchange_perp == 'okx':
                ...
                order = await hyperliquid.edit_order(order_id, symbol, 'post_only', side, quantity, price)
                return order['id']
        except Exception as e:
            if 'Cannot modify canceled or filled order' in str(e):
    ...
    def calculate_spreads_and_limits(self, latest_data, latest_row):
        ...
        entry_limit_price = latest_row[f'best_ask_price_hyperliquid']
        exit_limit_price = latest_row[f'best_bid_price_hyperliquid']
    ...
    async def main(self):
        ...
        self.adjustment_value = (self.get_latest_row(f"{self.exchange_perp.upper()}_PERP_{self.exchange_spot.upper()}_SPOT_{self.symbol}"))['best_bid_price_hyperliquid'] * 0.1
            ...
            if latest_data and len(latest_data) >= self.ma:
                ...
                try:
                    if self.exchange_perp == 'okx':
                        ...
                        trade_symbol = f'{self.symbol}/USDC:USDC'
...
bot_configs = [
    {'exchange_perp': 'hyperliquid', 'exchange_spot': 'bybit', 'symbol': 'CHILLGUY', 'notional_per_trade': 100, 'bot_id': 'CHILLGUY', 'max_notional': 10000, 'spreadThreshold': 0.1, 'side': 'sell'},
    {'exchange_perp': 'hyperliquid', 'exchange_spot': 'bybit', 'symbol': 'GOAT', 'notional_per_trade': 100,
     'bot_id': 'GOAT', 'max_notional': 10000, 'spreadThreshold': 0.05, 'side': 'sell'}
