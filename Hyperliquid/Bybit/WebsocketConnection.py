"""
Configured to run Bybit Spot, Hyperliquid Perp websockets
"""
import os
import csv
import json
...
# Update the URLs and stream types - replace OKX with Hyperliquid
EXCHANGE_WS_URLS = {
...
STREAM_TYPES = {
    "HYPERLIQUID_PERP": ["l2Book"],  # Hyperliquid's L2 orderbook stream
    "BINANCE_PERP": [
    ...
    "BYBIT_SPOT": ["orderbook.50."]
}

SUPPORT_EXCHANGES = ['OKX', 'BINANCE', 'HYPERLIQUID', 'GATE', 'BITGET', 'BYBIT']
    ...
    async def process_message(self, message):
        try:
            ...
            if 'channel' in data and data['channel'] == 'l2Book' and 'data' in data:
                book_data = data['data']
                if 'coin' in book_data and book_data['coin'] == self.symbol and 'levels' in book_data:
                    timestamp = int(book_data.get('time', time.time() * 1000))

                    # Hyperliquid sends bids and asks in separate arrays within levels
            ...
            logging.error(f"Error decoding Hyperliquid message: {e}")
        except Exception as e:
    ...
    async def process_message(self, message):
        ...
        if 'data' in data and len(data['data']) > 0:
            ...
            processed_data = {
                ...
                'bids': [[float(price), float(amount)] for price, amount in
                         list(self.ob[stream_type].bids.items())[:5]],
...
class CSVLogger:
    ...
    def log(self, spread_data):
        ...
        row = [
            ...
            spread_data['best_bid_price_hyperliquid'],
            spread_data['best_ask_price_hyperliquid'],
            spread_data['best_bid_price_bybit'],
            spread_data['best_ask_price_bybit'],
            spread_data['perp1_spot1_time_diff'],
            spread_data['perp1_timestamp'],
            spread_data['spot1_timestamp']
        ]
...
class SpreadCalculator:
    def __init__(self, stream1, stream2, symbol):
        ...
        self.orderbook_data = {
            ...
            'spot1': {'bids': [], 'asks': [], 'timestamp': None}
        }
    ...
    def calculate_spread_data(self):
        ...
        spot1_data = self.orderbook_data['spot1']

        if not perp1_data['bids'] or not spot1_data['bids']:
            return None

        perp1_best_bid = perp1_data['bids'][0][0]
        perp1_best_ask = perp1_data['asks'][0][0]
        spot1_best_bid = spot1_data['bids'][0][0]
        spot1_best_ask = spot1_data['asks'][0][0]
        perp1_spot1_time_diff = abs(perp1_data['timestamp'] - spot1_data['timestamp'])

        entry_spread = (perp1_best_bid - spot1_best_ask) / spot1_best_ask * 100
        exit_spread = (perp1_best_ask - spot1_best_bid) / spot1_best_bid * 100

        return {
            'symbol': self.symbol,
            'timestamp': max(perp1_data['timestamp'], spot1_data['timestamp']),
            'perp1_exchange': self.stream1.exchange,
            'spot1_exchange': self.stream2.exchange,
            'entry_spread': round(entry_spread, 4),
            'exit_spread': round(exit_spread, 4),
            'best_bid_price_hyperliquid': perp1_best_bid,
            'best_ask_price_hyperliquid': perp1_best_ask,
            'best_bid_price_bybit': spot1_best_bid,
            'best_ask_price_bybit': spot1_best_ask,
            'impact_bid_price_hyperliquid': perp1_best_bid,
            'impact_ask_price_hyperliquid': perp1_best_ask,
            'impact_bid_price_bybit': spot1_best_bid,
            'impact_ask_price_bybit': spot1_best_ask,
            'okx_orderbook': perp1_data,
            'bybit_orderbook': spot1_data,
            'perp1_spot1_time_diff': perp1_spot1_time_diff,
            'perp1_timestamp': perp1_data['timestamp'],
            'spot1_timestamp': spot1_data['timestamp'],
...
async def main():
    ...
    while True:
        exchange1 = 'HYPERLIQUID'  # Replace OKX with HYPERLIQUID
        exchange2 = 'BYBIT'
