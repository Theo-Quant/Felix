...
logger = config.setup_logger('OKXHyperliquidBot')
...
class CSVLogger:
    ...
    def create_new_file(self):
        ...
        filename = os.path.join(onedrive_path, f"HyperliquidOKX_Bot_{self.bot_id}_{timestamp}.csv")
...
class TradingBot:
    def __init__(self, exchange_perp, exchange_spot, symbol, notional_per_trade, bot_id, max_notional, spreadThreshold, side):
        ...
        self.symbol = symbol
        self.params_key = f'HyperliquidOKX_bot_params_{bot_id}'
        self.default_params = {
    ...
    def update_params(self):
        ...
        if self.params['hyperliquid_mark_price'] != 0:
            self.mark_price = self.params['hyperliquid_mark_price']
    ...
    def generate_client_order_id():
        return 'HLArb' + ''.join(random.choices(string.ascii_letters + string.digits, k=10))
            ...
            if self.exchange_perp == 'okx':
                order = await okx.create_order(symbol, "post_only", side, quantity, price,
                                               params={'clientOrderId': client_order_id,
                                                       'marginMode': 'isolated'  # Add this line
                                                       })
                return order['id']
    ...
    def calculate_spreads_and_limits(self, latest_data, latest_row):
        ...
        entry_limit_price = latest_row[f'best_ask_price_okx']
        exit_limit_price = latest_row[f'best_bid_price_okx']
    ...
    async def main(self):
        ...
        self.adjustment_value = (self.get_latest_row(f'HyperliquidOKX_combined_data_{self.bot_id}'))['best_bid_price_hyperliquid'] * 0.1
        self.mark_price = (self.get_latest_row(f'HyperliquidOKX_combined_data_{self.bot_id}'))['best_bid_price_hyperliquid']
        ...
        while True:
            ...
            latest_data = self.get_latest_data(f'HyperliquidOKX_combined_data_{self.bot_id}', count=self.ma)
            latest_row = self.get_latest_row(f'HyperliquidOKX_combined_data_{self.bot_id}')
            ...
            if latest_data and len(latest_data) >= self.ma:
                ...
                try:
                    if self.exchange_perp == 'okx':
                        trade_symbol = f'{self.symbol}-USDT-250606' # Strictly for HYPE Expiry Future
                    else:
...
bot_configs = [
    {'exchange_perp': 'okx', 'exchange_spot': 'okx', 'symbol': 'HYPE', 'notional_per_trade': 50, 'bot_id': 'HYPE', 'max_notional': 200, 'spreadThreshold': 0, 'side': 'sell'}
]
