...
class CSVLogger:
    ...
    def create_new_file(self):
        ...
        onedrive_path = r"C:\Users\gordo\OneDrive\Documents\tradelogs"
...
class TradingBot:
    ...
    def update_params(self):
        ...
        if float(self.params['binance_mark_price']) > 0:
    ...
    def generate_client_order_id():
        return 'GMSpotPerpArb' + ''.join(random.choices(string.ascii_letters + string.digits, k=10))
    ...
    def calculate_ma(data, key):
        ...
        if len(recent_values) > 0:
            ...
            print(f'Length of recent values: {len(recent_values)}')
    ...
    def log_order_action(self, side, ma_entry_spread, ma_exit_spread, latest_row, entry_limit_price,
        ...
        if side == 'sell':
            print(
                f'(Position: ${round(self.get_current_position_size(), 2)} / ${round(self.max_notional, 2)} | {datetime.now()}) | Bot {self.bot_id} | {side} order | sell ma: {round(ma_entry_spread, 3)} | sell spread: {round(latest_row["entry_spread"], 3)} | limit order: {round(entry_limit_price, 6)} | bollinger bands: ({round(self.entry_bound, 5)}, {round(self.exit_bound, 5)})')
        elif side == 'buy':
            print(
                f'(Position: ${round(self.get_current_position_size(), 2)} / ${round(self.max_notional, 2)} | {datetime.now()}) | Bot {self.bot_id} | {side} order | buy ma: {round(ma_exit_spread, 3)} | buy spread: {round(latest_row["exit_spread"], 3)} | limit order: {round(exit_limit_price, 6)} | bollinger bands: ({round(self.entry_bound, 5)}, {round(self.exit_bound, 5)}))')
    ...
    async def main(self):
        ...
        while True:
            ...
            self.entry_bound, self.exit_bound = self.get_trend_data()
            ...
            if latest_data and len(latest_data) >= self.ma:
                ...
                try:
                    ...
                    if any(msg in str(e) for msg in ['Your order has already been filled or canceled',
                        ...
                        self.current_order_id = None
                    elif any(msg in str(e) for msg in ['modification requests', 'System is busy']):
                        logger.info(f'Pausing bot {self.symbol} for 5 seconds due to {e}')
                        ...
                        # Get the most recent frame (where the error occurred)
                        ...
                        logger.error('=' * 50)
                        logger.error(f'CRITICAL ERROR DETAILS:')
                        ...
                        logger.error(f'  Code Line: {code_line}')
                        logger.error(f'Error Type: {exc_type.__name__}')
                        logger.error(f'Error Message: {str(exc_value)}')
                        logger.error('Full Traceback:')
                        logger.error(''.join(traceback.format_tb(exc_traceback)))
                        logger.error('Context:')
                        logger.error(f'  Symbol: {self.symbol}')
                        logger.error(f'  Current Order ID: {self.current_order_id}')
                        logger.error(f'  Previous Side: {self.previous_side}')
                        logger.error(f'  Current Position Size: {self.get_current_position_size()}')
                        logger.error('=' * 50)
                        raise  # This will stop the bot
...
bot_configs = [
    {'exchange_perp': 'okx', 'exchange_spot': 'binance', 'symbol': 'NEIRO', 'notional_per_trade': 50, 'bot_id': 'NEIRO', 'max_notional': 500, 'spreadThreshold': 0.1, 'side': 'sell'}
]
