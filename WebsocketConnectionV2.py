...
symbols = ['MOODENG', 'NEIRO', 'MASK', 'APE', 'POPCAT', 'UMA', 'HMSTR', 'UXLINK', 'BIGTIME', 'NOT', 'TIA', 'OM', 'MKR', 'AAVE', 'JTO', 'MEW', 'ORDI', 'STORJ', 'TURBO', 'AR', 'TRB', 'ORBS', 'ALPHA', 'AUCTION', 'SOL', 'SUI', 'ZETA', 'TRX', 'ENJ', 'DOGS', 'TON', 'LQTY', 'RDNT', 'BNT', 'AGLD', 'TNSR']
...
def process_data(symbol):
    ...
    if latest_data[symbol]['binance']['time'] != 0 and okx_data_available:
        if rate_limiter.should_process(symbol):
            ...
            impact_bid_okx = calculate_impact_price(okx_latest['bids'], 100)
            impact_ask_okx = calculate_impact_price(okx_latest['asks'], 100)
