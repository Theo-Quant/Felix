...
logger = config.setup_logger('WebsocketHedge(1)')
...
async def perform_hedge_check():
    API_Hedge = APIHedge(config.HEDGE1)
...
class WebsocketHedge:
    ...
    async def process_order_update(self, order):
        ...
        if order.get('instId').replace('-USDT-SWAP', '') in (config.HEDGE1 + ['CAT']):
    ...
    async def hedge_on_binance(self, symbol, side, amount, cli_ord_id):
        ...
        # Make this change just for CAT
        if binance_symbol == 'CATUSDT':
            hedge_amount = round(amount * contract_size/1000, 5)
            binance_symbol = binance_symbol.replace('CAT', '1000CAT')
        else:
